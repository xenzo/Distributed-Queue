...
class SkipListSupport<K, V> extends LinkedDequeSupport<V> {
    ...
    private final transient HeadIndex<K, V> _bottomHeadIndex = new HeadIndex<>(NULL_NODE, null, null, 1);
    /** The topmost head index of the skiplist. */
    private final transient AtomicReference<HeadIndex<K, V>> _topmostHeadIndex = new AtomicReference<>(_bottomHeadIndex);
    /**
     ...
     */
    @SuppressWarnings("unchecked") private final Comparable<K> _comparable(final K key)
            throws ClassCastException {
        if (key == null) throw new NullPointerException();
        if (_comparator != null) return new ComparableUsingComparator<>(key, _comparator);
    ...
    Index<K, V> newIndex(final K key, final Node<V> node, final Index<K, V> down, final Index<K, V> right) {
        if (key == null) throw new NullPointerException();
        return node == null ? new Index<>(key, down, right) : new Index<>(key, node, down, right);
    }
    ...
    private final V _deleteNode(final Index<K, V> index) {
        Node<V> node = null;
        if (index == null || (node = index.node()) == null || node.element() == null) throw new NullPointerException();
     ...
     */
    private final class ComparableUsingComparator<I> implements Comparable<I> {
        /** The actual key. */
         ...
         * @param cmp the cmp
         */
        private ComparableUsingComparator(final I key, final Comparator<I> cmp) {
            this._actualKey = key;
            this._cmp = cmp;
        }
    ...
    private final class HeadIndex<S, T> extends Index<S, T> {
         ...
         * @param level the level
         */
        private HeadIndex(final Node<T> node, final Index<S, T> down, final Index<S, T> right, final int level) {
    ...
    private final class IndexIter extends SolidItr<Index<K, V>> {
        /** @InheritDoc */
        @Override public Index<K, V> next() {
            Index<K, V> next;
            do {
                next = _next;
                advance();
    ...
    private final class EntryIter extends SolidItr<Entry<K, V>> {
         ...
         * @InheritDoc
         */
        @Override public final Entry<K, V> next() {
            Index<K, V> next;
            do {
                next = _next;
                advance();
