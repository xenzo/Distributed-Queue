...
// Lifecycle
        ...
        @Override public void destroy() {
            ...
            if (_changeQueue != null) {
                ...
                _changeQueue = null;
                logger.logp(WARNING, getClass().getName(), "destroy", "not processed request remanins : " + queue.size(), queue);
                queue.clear();
            }
        }
        ...
        private final void _processChangeRequest() {
            ...
            while ((req = _changeQueue.poll()) != null) {
                ...
                switch (req.type) {
                case ADD_OPS:
                    if (key != null && key.isValid()) key.interestOps(key.interestOps() | (req.channel.validOps() & req.ops));
                    break;
                case CHANGE_OPS:
                    if (key != null && key.isValid()) key.interestOps(req.channel.validOps() & req.ops);
                    break;
                case REMOVE_OPS:
                    if (key != null && key.isValid()) key.interestOps(key.interestOps() ^ (key.interestOps() & req.ops));
                    break;
                case REGISTER:
                    try {
