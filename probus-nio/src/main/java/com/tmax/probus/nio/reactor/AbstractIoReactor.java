...
public abstract class AbstractIoReactor extends AbstractReactor {
     ...
     */
    protected ISession createSession(final ServerSocketChannel serverChannel, final SocketChannel channel) {
        return null;
    }

    // (non-Javadoc)
    // @see com.tmax.probus.nio.reactor.AbstractReactor#handleAccept(java.nio.channels.SelectionKey)
    @Override protected final void handleAccept(final SelectionKey key) {
    ...
    @Override protected final void handleConnect(final SelectionKey key) {
    ...
    @Override protected final void handleRead(final SelectionKey key) {
    ...
    @Override protected final void handleWrite(final SelectionKey key) {
     ...
     */
    protected abstract SelectableChannel processAccept(SelectionKey key);
     ...
     * @return the selectable channel
     */
    protected abstract SelectableChannel processConnect(SelectionKey key);
     ...
     */
    protected boolean processRead(final SelectionKey key) {
        ...
        try {
            ...
            session.onMessageRead();
        } catch (final IOException ex) {
            logger.logp(SEVERE, "AbstractIoReactor", "processRead(SelectionKey)", "", ex);
        }
        if (logger.isLoggable(FINER)) logger.exiting("AbstractIoReactor", "processRead(SelectionKey)", "end - return value=" + false);
        return false;
    }
     ...
     * @return true, if successful
     */
    protected boolean processWrite(final SelectionKey key) {
        ...
        try {
            ...
            while (!queue.isEmpty()) {
                ...
                try {
                    ...
                    logger.logp(SEVERE, "IOReactor", "processWrite(SelectionKey)", "", ex);
                }
